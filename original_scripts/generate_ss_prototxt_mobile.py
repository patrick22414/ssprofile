    ...
    if method == "train_test":
        net.data, net.label = Layers.ImageData(
            ...
            transform_param=dict(
                mirror=True, crop_size=32, mean_value=[104, 117, 123],
            ),
        ...
        net.data, net.label = Layers.ImageData(
            name="cifar",
            ntop=2,
            transform_param=dict(mirror=False, mean_value=[104, 117, 123],),
            image_data_param=dict(
...
MOBILE_CHANNEL = [16] * 1 + [24] * 2 + [32] * 2 + [64] * 2 + [128] * 2 + [256] * 1
MOBILE_STRIDE = [1] + [2] + [1] + [2] + [1] + [2] + [1] + [2] + [1] + [1]

# function for generate mobilenet block
# 1x1conv + dwconv + 1x1conv + perhaps shotcut
def generate_mobile_block(
    input_blob_name,
    layer_name,
    input_size,
    input_channel,
...
def generate_mobile(method, CHOICE_LIST):
    ...
    for i in range(10):
        ...
        input_blob_name, input_size, input_channel = generate_mobile_block(
            input_blob_name,
            layer_name,
            input_size,
            input_channel,
    ...
    input_blob_name = generate_fc(input_blob_name, "FC", 10, False, False)
    if method == "train_test":
        ## loss and accuracy
...
for CHOICE_LIST in CHOICE_TOTAL:
    ...
    cmd = (
        "~/caffe_dev/build/tools/caffe train -solver %s/finetune_solver.prototxt -weights typical/model/baseline.model -gpu %d >& %s/finetune.log &"
